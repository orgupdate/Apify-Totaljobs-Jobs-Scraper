...
The most efficient way to extract job listings directly from TotalJobs Jobs search results.

[View on Apify Store](#)

## 📖 Overview

The **TotalJobs Jobs Scraper** is a powerful data extraction tool designed to aggregate job listings from the TotalJobs Jobs search engine.
...
- **🌍 Multi-Source Aggregation** – Access listings from thousands of job boards via a single TotalJobs search query.
...
|-----------|------|----------|-------------|---------|--------------|
| `countryName` | String | No | The country context for the TotalJobs Search domain | "all" | "usa", "uk", "india", etc. |
...
| `companyName` | String | No | Filter listings to a specific employer | - | Any company name (e.g., "TotalJobs", "Microsoft") |
    ...
    [
      {
        ...
        "date": "2025-03-25",
        "job_type": "Full-time",
        "URL": "https://www.TotalJobs.com/search?..."
      }
    ]
...
- **posted_via:** The original platform where TotalJobs found the job (e.g., Monster, Greenhouse, LinkedIn).
...
- **TotalJobs Sheets:** Automatically save new job listings into a spreadsheet for analysis.
