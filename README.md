# 🚀 Totaljobs Jobs Scraper – Scrape Totaljobs Jobs to JSON, CSV & Excel

Scrape and aggregate job listings from **Totaljobs** (totaljobs.com) search results – job title, company, location, salary, posting date and a direct apply link – and export them to JSON, CSV, Excel or an API in a single run.

[View on Apify Store](#)

## 📖 Overview

The **Totaljobs Jobs Scraper** is a data-extraction tool that turns Totaljobs job search results into structured data. Totaljobs is one of the UK's biggest job boards, with roles nationwide across all industries.
...
- **🔍 Full-Text Job Search** – Search Totaljobs by keyword, job title or skill and walk every result page automatically.
...
```
https://mcp.apify.com/?tools=actors,docs,orgupdate/totaljobs-jobs-scraper
```
...
With the server connected, your agent can search Totaljobs listings by title and city, compare postings across employers, and pull salary and apply links into a single structured answer.
...
| `companyName` | String | No | Restrict results to a single employer (e.g. `"Tesco"`). | - | Any company name |
...
Not every Totaljobs listing includes every field – `salary`, `job_type` and exact `date` vary by posting – so handle optional fields gracefully in your integration.
...
Run this Actor from the Apify API with the official clients. Replace `orgupdate/totaljobs-jobs-scraper` with the real Actor ID from the Actor page.
...
run = client.actor("orgupdate/totaljobs-jobs-scraper").call(run_input=run_input)
...
const run = await client.actor('orgupdate/totaljobs-jobs-scraper').call(input);
...
- **Find remote Totaljobs jobs via MCP** – let an AI agent pull remote listings with company, source and direct apply links.
- **Track Tesco's open roles** – monitor one employer's postings on Totaljobs on a daily schedule.
- **Jobs posted in the last 3 days** – recent Totaljobs listings filtered by `datePosted`, feeding a database.
- **Salary benchmark by city** – aggregate Totaljobs pay ranges for one job title across multiple locations.

## 💡 Use Cases

- **Job boards & aggregators** – Populate a niche board with fresh Totaljobs listings, no manual entry.
...
Some queries genuinely have few listings on Totaljobs. Broaden the keyword (e.g. `"engineer"` instead of `"senior backend engineer"`), widen `datePosted`, and raise `pagesToFetch`.
